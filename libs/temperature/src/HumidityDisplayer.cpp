    ...
    _layout(this)
{
    ...
    _humidityValue.setFont(defFont);
    _humidityValue.setMinimumWidth(120);
    _humidityValue.setAlignment(Qt::AlignHCenter | Qt::AlignVCenter);
    _layout.addWidget(&_humidityValue);
...
HumidityDisplayer::~HumidityDisplayer()
{
}

void HumidityDisplayer::paintEvent(QPaintEvent *event)
{
    // Background.
    QPainter qp(this);
...
{
    QLocale locale;
    _humidityValue.setText(locale.toString(humidity * 100.0, 'f', 1)+" %");
}

void HumidityDisplayer::unSetHumidity()
{
    _humidityValue.setText("-");
}

void HumidityDisplayer::translate()
{
    update();
}
...
void HumidityDisplayer::statesChanged()
{
    bool displayView = false;

    // Humidity view.
    if (StatesManager::instance()->viewType() == StatesManager::DisplayCondensationResults || StatesManager::instance()->viewType() == StatesManager::DisplayHumidityResults)
        displayView = true;

    // Temperature view & model type is humidity & license permits humidity.
    Model   *model   = StatesManager::instance()->currentModel();

    if (model && StatesManager::instance()->viewType() == StatesManager::DisplayResults)
    {
        if (model->computationType() == Model::GlaserCondensation)
            displayView = true;
    }

    setVisible(displayView);
}
