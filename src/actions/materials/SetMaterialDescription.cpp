...
SetMaterialDescription::SetMaterialDescription(Material *material, const std::string &description)
{
    _alterResults = false;

    _materialId=material->id();
    _newDescription=description;
    _oldDescription=material->description();
}
...
void SetMaterialDescription::exec()
{
    Material *material=Materials::instance()->materialFromId(_materialId);
    if (!material)
        return;
    
    material->setDescription(_newDescription);
    Materials::instance()->materialModified(_materialId);
}

void SetMaterialDescription::undo()
{
    Material *material=Materials::instance()->materialFromId(_materialId);
    if (!material)
        return;
    
    material->setDescription(_oldDescription);
    Materials::instance()->materialModified(_materialId);
}
