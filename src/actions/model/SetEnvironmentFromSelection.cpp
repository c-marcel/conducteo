...
SetEnvironmentFromSelection::SetEnvironmentFromSelection(Model *model, const std::string &environmentName, double x, double y, double width, double height):
    _model(model),
...
SetEnvironmentFromSelection::~SetEnvironmentFromSelection()
{
}

void SetEnvironmentFromSelection::exec()
{
    storePhysics(_model);
    ...
    for (unsigned int i=0 ; i<surfaces.size() ; i++)
    {
        ...
        surface->setEnvironment(Environments::instance()->environmentFromName(_environment));
    }
}

void SetEnvironmentFromSelection::undo()
{
    restorePhysics(_model);
}

bool SetEnvironmentFromSelection::isValid() const
{
    return (availableSurfaces().size() > 0);
}

std::vector<Surface *> SetEnvironmentFromSelection::availableSurfaces() const
{
