...
SendToFrontVolume::SendToFrontVolume(Model *model, Volume *volume):
    _model(model),
    _volumeId(volume->id()),
    _stackPosition(0)
{
...
SendToFrontVolume::~SendToFrontVolume()
{
}

void SendToFrontVolume::exec()
{
    storePhysics(_model);
    Volume *volume=_model->getVolumeById(_volumeId);
    if (volume)
    {
        _model->setVolumeStackPosition(volume, _model->std::vector<Volume*>::size()-1);
        _model->recreateSurfaces();
    }
}
void SendToFrontVolume::undo()
{
    Volume *volume=_model->getVolumeById(_volumeId);
    if (volume)
    {
